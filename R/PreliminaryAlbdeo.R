...
ext(twi) <- ext(final_extent)
ext(aspect) <- ext(final_extent)
ext(breakpoints) <- ext(final_extent)
ext(breakpoints1) <- ext(final_extent)
...
envistack <- c(canopy_height_model, topo, tpi, tri, slope, aspect, twi) #group together all layers with the same res/extent
breakpoints < breakpoints #placeholder to keep track, we need to do the same procedure on breakpoints as we do on envistack
...
breakpoints2 <- resample(breakpoints, vegstack)
breakpoints1 <- resample(breakpoints1, vegstack)
...
DT_box <- stack_DT %>%
  ...
  theme_classic()
plot_grid(All_box, nonwoody_box, ET_box, DT_box, DTSA_box, DTSW_box, ncol = 2)
