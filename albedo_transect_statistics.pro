PRO Albedo_Transect_Statistics
...
out_dir = 'G:\MyWork\Albedo_Scaling\Data_Analysis\transect_analysis'
file_mkdir, out_dir
...
;*************************************** LODA DATA ********************************************;
; load transect roi
roi_dir = 'G:\MyWork\Albedo_Scaling\Data_Analysis\transect'
roi_file_dir = file_search(roi_dir, 'transect_img.hdr')
...
data_combn_raster = EnviLayerStackRaster([pft_cover_raster, topo_dem_raster, topo_aspect_raster, topo_slope_raster, topo_hillslope_raster, $
...
x_max = max(transect_data_combn[0,*])
x_min = min(transect_data_combn[0,*])

transect_bin_mean = []
for x=x_min+3, x_max-3, 7 do begin
  x_loc = transect_data_combn[0,*]
  bin_loc = where(x_loc gt (x-2) and x_loc le (x+2))
  bin_data = transect_data_combn[*, bin_loc]
  bin_mean = mean(bin_data, dimension = 2)
  
  transect_bin_mean = [[transect_bin_mean],[bin_mean]]
endfor

headers = ['x', 'y', 'EVT', 'DTSA' , 'DTSW', 'DTSO', 'DLS', 'DDS', 'EVS', 'FOR', 'DGR', 'WGR', 'MOS',$
...
write_csv, out_name, transect_bin_mean, header = headers
END